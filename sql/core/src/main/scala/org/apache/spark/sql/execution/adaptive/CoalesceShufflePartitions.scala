...
case class CoalesceShufflePartitions(session: SparkSession) extends Rule[SparkPlan] {
  ...
  override def apply(plan: SparkPlan): SparkPlan = {
    ...
    if (!plan.collectLeaves().forall(_.isInstanceOf[QueryStageExec])
      ...
      // have same number of output partitions. 如果不是所有的叶节点都是查询阶段，那么减少shuffle分区的数量是不安全的，因为我们可能会打破spark计划的所有子节点具有相同数量的输出分区的假设。
      return plan
    }
    ...
    // ShuffleExchanges introduced by repartition do not support changing the number of partitions.重分区引入的ShuffleExchanges不支持更改分区数量。只有在所有ShuffleExchanges都支持的情况下，我们才会在这个阶段更改分区的数量。
    // We change the number of partitions in the stage only if all the ShuffleExchanges support it.如用户repartition操作将不会分区合并
    if (!shuffleStages.forall(_.shuffle.canChangeNumPartitions)) {
      ...
      // we should skip it when calculating the `partitionStartIndices`. 当输入RDD有0个分区时，ShuffleQueryStageExecmapStats '返回None，我们应该在计算' partitionStartIndices '时跳过它。
      val validMetrics = shuffleStages.flatMap(_.mapStats)

      // We may have different pre-shuffle partition numbers, don't reduce shuffle partition number
      // in that case. For example when we union fully aggregated data (data is arranged to a single 如果多个task进行shuffle，且task有不同的分区数的话
      // partition) and a result of a SortMergeJoin (multiple partitions).我们可能有不同的预洗牌分区号，在这种情况下不要减少洗牌分区号。例如，当我们将完全聚合的数据(数据被安排到单个分区)和SortMergeJoin的结果(多个分区)结合在一起时。
      val distinctNumPreShufflePartitions =
        validMetrics.map(stats => stats.bytesByPartitionId.length).distinct
      if (validMetrics.nonEmpty && distinctNumPreShufflePartitions.length == 1) {
        // We fall back to Spark default parallelism if the minimum number of coalesced partitions
        // is not set, so to avoid perf regressions compared to no coalescing. 如果没有设置合并分区的最小数量，我们将回到Spark默认并行度，以避免与没有合并相比的性能退化

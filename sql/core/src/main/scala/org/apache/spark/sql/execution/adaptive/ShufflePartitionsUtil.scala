...
object ShufflePartitionsUtil extends Logging {
  ...
  def coalescePartitions(
    ...
    // `advisoryTargetSize` as the target size of a coalesced task. 如果“minNumPartitions”非常大
    // 我们可能需要使用一个小于“advisoryTargetSize”的值作为合并任务的目标大小。 当前shuffle的总分区数内的数据总大小
    ...
    val maxTargetSize = math.max(
      // 求出每个分区的平均大小 如果inputSize=1000M 10分区  而设置advisoryTargetSize为200M 则通过一下计算会排除200M这个设置
    ...
    // 我们期望shuffle分区数量的单一值的原因是，当我们添加Exchanges时，我们使用静态设置设置了shuffle分区的数量
    // (i.e. map output partitions).
    // (即映射输出分区)，即' spark.sql.shuffle.partitions '的值。即使两个输入rdd拥有不同数量的分区，它们也将拥有相同数量的shuffle分区
    assert(
    ...
    while (i < numPartitions) {
      // 我们从所有shuffle中计算第i个shuffle分区的总大小 对于每个task中的每个相邻分区合并，直到不大于targetSize
